...
// 中文说明：去掉标签字符串首尾空白。
std::string trim_label_string(const std::string &label)
{
    const auto start = label.find_first_not_of(" \t\n\r\f\v");
...
struct LabelInvertedIndex
{
    size_t total_points = 0;
    std::unordered_map<std::string, std::vector<uint32_t>> label_to_points;

    // 中文说明：从 labels.txt 构建 label -> point_id 倒排索引。
    void build_from_labels_file(const std::string &label_file)
    {
        label_to_points.clear();
        total_points = 0;

        std::ifstream file(label_file);
        if (!file.is_open())
        {
            throw diskann::ANNException("Failed to open label file for inverted index", -1, __FUNCSIG__, __FILE__,
                                        __LINE__);
        }
        ...
        while (std::getline(file, line))
        {
            ...
            while (std::getline(ss, label, ','))
            {
                ...
                if (!label.empty())
                {
                    label_to_points[label].push_back(static_cast<uint32_t>(total_points));
                }
            }
            total_points++;
        }
    ...
    // 中文说明：对 OR 查询标签集合，统计满足任一属性的向量占全集比例（全局选择率）。
    double compute_or_selectivity(const std::vector<std::string> &query_labels) const
    {
        if (total_points == 0 || query_labels.empty())
        {
            return 0.0;
        }

        tsl::robin_set<uint32_t> valid_points;
        valid_points.reserve(query_labels.size() * 16);
        for (const auto &label : query_labels)
        {
            const auto it = label_to_points.find(label);
            ...
            {
                valid_points.insert(point_id);
            }
        }

        return static_cast<double>(valid_points.size()) / static_cast<double>(total_points);
    }
};
...
int search_memory_index(diskann::Metric &metric, const std::string &index_path, const std::string &result_path_prefix,
                        ...
                        const std::string &query_filters_file, const std::string &label_file,
                        ...
                        const uint32_t label_projection_dim)
{
    ...
    LabelInvertedIndex inverted_index;
    inverted_index.build_from_labels_file(label_file);
    std::cout << "Label inverted index built from " << label_file << ", total_points=" << inverted_index.total_points
              << ", unique_labels=" << inverted_index.label_to_points.size() << std::endl;
    ...
    // 中文说明：全局选择率与拓展属性数在计时外预计算，不计入 QPS / Latency。
    std::vector<std::vector<std::string>> normalized_query_labels = query_label_sets;
    for (auto &labels : normalized_query_labels)
    {
        std::sort(labels.begin(), labels.end());
        labels.erase(std::unique(labels.begin(), labels.end()), labels.end());
    }

    std::vector<uint32_t> adaptive_expand_k_per_query(query_num, expand_labels_k);
    if (expand_labels_k > 0)
    {
        for (size_t query_idx = 0; query_idx < query_num; query_idx++)
        {
            if (normalized_query_labels[query_idx].empty())
            {
                continue;
            }

            double R = inverted_index.compute_or_selectivity(normalized_query_labels[query_idx]);
            R = std::max(0.0, std::min(1.0, R));
            const double factor = std::max(0.0, 1.0 - R);
            uint32_t adaptive_expand_k =
                static_cast<uint32_t>(std::ceil(static_cast<double>(expand_labels_k) * factor));
            adaptive_expand_k_per_query[query_idx] = std::min<uint32_t>(adaptive_expand_k, expand_labels_k);
        }
    }
    std::cout << "Global selectivity precomputed for " << query_num << " queries." << std::endl;
...
#pragma omp parallel for schedule(dynamic, 1)
        for (int64_t i = 0; i < (int64_t)query_num; i++)
        {
            ...
            const auto &current_query_labels = normalized_query_labels[(size_t)i];
            const uint32_t adaptive_expand_k = adaptive_expand_k_per_query[(size_t)i];
            ...
            if (dynamic && tags)
            {
                auto retval = typed_index->search_with_filter_label_group_tags(
                    ...
                    static_cast<int>(adaptive_expand_k), false, nullptr);
...
int main(int argc, char **argv)
{
    ...
    po::options_description desc{program_options_utils::make_program_description(
        "test_global_selectivity",
        "Searches in-memory DiskANN indexes with global OR selectivity for adaptive label expansion")};
    try
    {
        ...
        required_configs.add_options()(
            "label_file", po::value<std::string>(&label_file)->required(),
            "Path to base labels file used to build the global label inverted index.");
        ...
        if (label_type == "ushort")
        {
            if (data_type == std::string("int8"))
            {
                return search_memory_index<int8_t, uint16_t>(
                    ...
                    Lvec, dynamic, tags, show_qps_per_thread, query_filters_file, label_file, fail_if_recall_below,
        ...
        else
        {
            if (data_type == std::string("int8"))
            {
                return search_memory_index<int8_t>(metric, index_path_prefix, result_path, query_file, gt_file,
                                                   ...
                                                   show_qps_per_thread, query_filters_file, label_file,
                                                   fail_if_recall_below, expand_labels_k, label_projection_dim);
            }
            else if (data_type == std::string("uint8"))
            {
