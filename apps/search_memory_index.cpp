...
int search_memory_index(diskann::Metric &metric, const std::string &index_path, const std::string &result_path_prefix,
                        ...
                        const uint32_t expand_labels_k)
{
    using TagT = uint32_t;
    // Load the query file
    T *query = nullptr;
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        // 新增 CLI: expand_labels_k（必须在合并到 desc 之前注册）
        ...
        if (!query_filters.empty() && label_type == "ushort")
        {
            if (data_type == std::string("int8"))
            {
                return search_memory_index<int8_t, uint16_t>(
                    ...
                    Lvec, dynamic, tags, show_qps_per_thread, query_filters, fail_if_recall_below, expand_labels_k);
            }
